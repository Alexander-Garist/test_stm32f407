...
int main()
{
	...
	USART_Enable(USART1, GPIOA, 9, GPIOA, 10, 115200);	// Включение модуля USART1; PA9 Tx; PA10 Rx; Baudrate 115200
	...
	*/
	...
	uint8_t OUT_SIGNAL_AMPLITUDE = 16;				// 16/256 = 6.25% от максимальной амплитуды
	uint16_t OUT_SIGNAL_MODE = AD9833_SQUARE_MODE;	// Меандр
	...
	*/
	...
	GPIO_set_HIGH(GPIOD, 12);								// Загорелся зеленый светодиод: генерируется исходный сигнал (меандр 100 кГц 6.25% амплитуды)
	delay_ms(10000);
	GPIO_set_LOW(GPIOD, 12);

	// Изменение параметров выходного сигнала
	// Следующие 10 секунд: меандр с частотой 1 кГц
	AD9833_SetFrequency(SPI1, 1000);						// Изменение частоты сигнала
	GPIO_set_HIGH(GPIOD, 13);								// Загорелся оранжевый светодиод: генерируется измененный сигнал (меандр 1 кГц 6.25% амплитуды)
	delay_ms(10000);
	GPIO_set_LOW(GPIOD, 13);

	// Следующие 10 секунд: пила с частотой 1 кГц
	AD9833_SetOutputMode(SPI1, AD9833_TRIANGLE_MODE);		// Изменение формы сигнала
	GPIO_set_HIGH(GPIOD, 15);								// Загорелся синий светодиод: генерируется измененный сигнал (пила 1 кГц 6.25% амплитуды)
	delay_ms(10000);
	GPIO_set_LOW(GPIOD, 15);

	// Следующие 10 секунд: пила с частотой 1 кГц и большей амплитудой
	AD9833_SetAmplitude(SPI1, 32);							// Изменение амплитуды сигнала на 12.5% от максимума
	GPIO_set_HIGH(GPIOD, 14);								// Загорелся красный светодиод: генерируется измененный сигнал (пила 1 кГц 12.5% амплитуды)
	delay_ms(10000);
	GPIO_set_LOW(GPIOD, 14);

	// Дальнейший выходной сигнал: синусоида 100 кГц 50% амплитуды
